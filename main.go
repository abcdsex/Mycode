...
func main() {
	listener, err := net.Listen("tcp", "127.0.0.1:8011") //持续监听地址和端口
	if err != nil {
	...
	//启动管家go协程，不断监听全局channel————message
	go manager()

	for {
		conn, err := listener.Accept() //返回一个新的套字节，客户端通信
		if err != nil {
			fmt.Println("listener.Accept error:", err)
			return
		}
		fmt.Printf("地址为[%v]的客户端已连接成功\n", conn.RemoteAddr()) //提示地址和端口
...
func HandleConnect(conn net.Conn) {
	...
	_, _ = conn.Write([]byte("你好," + userName + ",欢迎来到聊天室,请畅所欲言！")) //像客户端发送消息
	//广播通知。遍历map
	go func() {
...
func manager() {
	for {
		select {
		case msg := <-message:
			for _, v := range onlineUsers { //向每个在线用户发送信息
				v.C <- msg
			}
		}
	}
